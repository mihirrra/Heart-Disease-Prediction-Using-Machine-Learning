...
Task 10: Manually format and write processed data to a new CSV file WITHOUT
        df.to_csv() / csv.writer, with storage-efficient formatting.
Task 11: Histograms for all numeric features WITHOUT ax.hist()/plt.hist() —
        manual bin-edge calculation, manual frequency counting, drawn with
        raw ax.bar().
Task 12: Boxplots WITHOUT ax.boxplot() — quartiles/IQR/fences/outliers all
        computed manually and drawn using raw Matplotlib primitives
        (ax.plot/ax.hlines/ax.add_patch/ax.scatter).

Note: Streamlit itself is used only for the UI (widgets, layout, tables).
All data loading, parsing, statistics, dedup, encoding, scaling, feature
engineering, export, and plotting logic are implemented from scratch in
...
# ============================================================================
# TASK 10: MANUAL CSV EXPORT (no df.to_csv() / csv.writer)
# ============================================================================

def format_value_for_csv(v):
    """Storage-efficient manual formatting: ints stay bare, floats drop
    trailing zeros, None becomes empty field, strings get quoted only
    when they actually contain the delimiter/quote/newline (minimizes
    bytes written vs. blanket-quoting everything)."""
    if v is None:
        return ""
...
def build_csv_text_manual(header, rows):
    """Manually builds the full CSV text: header line + one line per row,
    joining fields with ',' and lines with '\\n' — no csv module, no
    pandas.to_csv(). Uses a list + ''.join for efficiency instead of
    repeated string concatenation."""
    lines = []
    lines.append(",".join(format_value_for_csv(h) for h in header))
    for row in rows:
        lines.append(",".join(format_value_for_csv(v) for v in row))
    return "\n".join(lines) + "\n"


# ============================================================================
# TASK 11: MANUAL HISTOGRAMS (no ax.hist()/plt.hist() — manual binning)
# ============================================================================

def compute_bins_manual(values, n_bins=15):
    """Manually compute equal-width bin edges (no numpy.histogram_bin_edges)."""
    mn, mx = manual_min(values), manual_max(values)
    if mn == mx:
        return [mn, mn + 1], [len(values)]
    width = (mx - mn) / n_bins
    edges = [mn + i * width for i in range(n_bins + 1)]
    return edges, width


def compute_frequencies_manual(values, edges):
    """Manually count how many values fall in each [edge[i], edge[i+1])
    bucket, with the last bucket closed on both ends."""
    n_bins = len(edges) - 1
    freqs = [0] * n_bins
    for v in values:
        placed = False
        for i in range(n_bins):
            ...
            if i == n_bins - 1:
                ...
                if lo <= v < hi:
                    ...
                    break
        if not placed:
            # value exactly at global max or a float-precision edge case
            if v >= edges[-1]:
                freqs[-1] += 1
            elif v < edges[0]:
                freqs[0] += 1
...
def draw_manual_histogram(values, col_name, n_bins=15, color="#5B8DEF"):
    """Draws a histogram using raw ax.bar() only — bin edges and
    frequencies are both computed by hand above, no ax.hist()/plt.hist()."""
    edges, width = compute_bins_manual(values, n_bins)
    freqs = compute_frequencies_manual(values, edges)
    centers = [(edges[i] + edges[i + 1]) / 2 for i in range(len(edges) - 1)]
    bar_width = (edges[1] - edges[0]) * 0.92 if len(edges) > 1 else 1

    fig, ax = plt.subplots(figsize=(4.2, 3.2))
    ax.bar(centers, freqs, width=bar_width, color=color, edgecolor="white", linewidth=0.6, alpha=0.9)
    ax.set_title(col_name, fontsize=9, fontweight="bold")
    ax.tick_params(labelsize=7)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    return fig, edges, freqs


# ============================================================================
# TASK 12: MANUAL BOXPLOTS (no ax.boxplot() — manual quartiles/IQR/fences,
# drawn with raw Matplotlib primitives)
# ============================================================================

def compute_boxplot_stats_manual(values):
    """Manually computes Q1, median, Q3, IQR, whisker fences, and flags
    outliers beyond the fences (Tukey's method), all by hand."""
    sorted_vals = manual_sort(values)
    q1 = manual_percentile(sorted_vals, 25)
    q2 = manual_percentile(sorted_vals, 50)
    q3 = manual_percentile(sorted_vals, 75)
    iqr = q3 - q1
    lower_fence = q1 - 1.5 * iqr
    upper_fence = q3 + 1.5 * iqr

    inliers = [v for v in sorted_vals if lower_fence <= v <= upper_fence]
    outliers = [v for v in sorted_vals if v < lower_fence or v > upper_fence]

    whisker_low = manual_min(inliers) if inliers else q1
    whisker_high = manual_max(inliers) if inliers else q3
...
def draw_manual_boxplot(stats_by_col, labels, colors=None):
    """Draws boxplots from scratch using ax.plot/ax.fill_between/ax.scatter
    only — no ax.boxplot() / plt.boxplot() anywhere."""
    if colors is None:
        colors = ["#5B8DEF"] * len(labels)
    fig, ax = plt.subplots(figsize=(max(6, len(labels) * 1.0), 5))
    box_half_width = 0.32

    for i, (label, stats) in enumerate(zip(labels, stats_by_col.values())):
        x = i + 1
        q1, med, q3 = stats["q1"], stats["median"], stats["q3"]
        wl, wh = stats["whisker_low"], stats["whisker_high"]

        # box (Q1-Q3) drawn as a filled rectangle via fill_between
        ax.fill_between([x - box_half_width, x + box_half_width], q1, q3,
                         color=colors[i % len(colors)], alpha=0.55, edgecolor="#2E86C1", linewidth=1.2, zorder=2)
        # median line
        ax.plot([x - box_half_width, x + box_half_width], [med, med], color="#E74C3C", linewidth=2, zorder=3)
        # whiskers
        ax.plot([x, x], [q3, wh], color="#2E86C1", linewidth=1.2, zorder=1)
        ax.plot([x, x], [q1, wl], color="#2E86C1", linewidth=1.2, zorder=1)
        # whisker caps
        cap_w = box_half_width * 0.5
        ax.plot([x - cap_w, x + cap_w], [wh, wh], color="#2E86C1", linewidth=1.2, zorder=1)
        ax.plot([x - cap_w, x + cap_w], [wl, wl], color="#2E86C1", linewidth=1.2, zorder=1)
        # outliers as scattered points
        if stats["outliers"]:
            ax.scatter([x] * len(stats["outliers"]), stats["outliers"],
                       color="#D0021B", s=14, zorder=4, marker="o")

    ax.set_xticks(range(1, len(labels) + 1))
    ax.set_xticklabels(labels, rotation=40, ha="right", fontsize=8)
    ax.set_title("Manual Boxplots — Quartiles, IQR Fences & Outliers", fontsize=12, fontweight="bold")
    ax.set_ylabel("Value", fontsize=10)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
...
tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8, tab9, tab10, tab11, tab12 = st.tabs([
    ...
    "💾 Task 10: CSV Export",
    "📊 Task 11: Manual Histograms",
    "📦 Task 12: Manual Boxplots",
])
...
# ---------------- TASK 10 ----------------
with tab10:
    st.subheader("Manual CSV Export (No `df.to_csv()` / `csv.writer`)")
    st.caption("The CSV text is built entirely by hand: manual per-value formatting (bare ints, trimmed floats, minimal quoting) and manual line joining — no pandas export function, no `csv` module.")

    st.markdown("Choose which processing to apply before export (optional), then generate the file:")
    apply_dedup = st.checkbox("Remove duplicates (Task 4 logic)", value=False)
    apply_impute_col = st.selectbox("Impute a column's missing values before export (optional)",
                                     options=["None"] + [c for c, n in find_missing(header, rows)[0].items() if n > 0])

    export_rows = [row[:] for row in rows]

    if apply_impute_col != "None":
        col_i = header.index(apply_impute_col)
        col_vals_export = get_column(export_rows, col_i)
        nums_export = numeric_values(col_vals_export)
        if nums_export:
            med_export = manual_median(nums_export)
            export_rows = impute_median(export_rows, col_i, med_export)
        else:
            mode_export = manual_mode(col_vals_export)
            export_rows = impute_mode(export_rows, col_i, mode_export)

    if apply_dedup:
        dup_idx_export, _ = find_duplicates_manual(export_rows)
        export_rows = remove_duplicates_manual(export_rows, dup_idx_export)

    csv_text = build_csv_text_manual(header, export_rows)
    size_bytes = len(csv_text.encode("utf-8"))

    c1, c2 = st.columns(2)
    c1.metric("Rows to export", len(export_rows))
    c2.metric("Output file size", f"{size_bytes:,} bytes")

    st.write("**Preview (first 10 lines of the manually built CSV text):**")
    st.code("\n".join(csv_text.split("\n")[:11]), language="text")

    st.download_button(
        label="⬇️ Download processed_heart.csv",
        data=csv_text,
        file_name="processed_heart.csv",
        mime="text/csv",
    )

    with st.expander("📋 Storage-efficiency notes"):
        st.markdown("""
- Integers are written bare (`52`, not `52.0`) and floats have trailing zeros trimmed (`3.1`, not `3.100000`) — this alone can meaningfully cut file size on a numeric-heavy dataset like this one.
- Fields are only wrapped in quotes when they actually contain a comma, quote, or newline — most cells here are plain numbers, so blanket-quoting (which some naive writers do) is avoided.
- The whole file is built as a list of line-strings and joined once with `"\\n".join(...)`, avoiding the O(n²) cost of repeatedly concatenating one giant string.
        """)

# ---------------- TASK 11 ----------------
with tab11:
    st.subheader("Manual Histograms for All Numerical Features")
    st.caption("No `ax.hist()` / `plt.hist()` anywhere — bin edges are computed manually (equal-width), each value is manually counted into its bin, and bars are drawn with raw `ax.bar()`.")

    numeric_cols_t11 = get_numeric_columns(header, rows)
    n_bins_choice = st.slider("Number of bins", min_value=5, max_value=30, value=15, key="bins_t11")

    if not numeric_cols_t11:
        st.warning("No numeric columns found.")
    else:
        n_cols_grid = 3
        n = len(numeric_cols_t11)
        n_rows_grid = math.ceil(n / n_cols_grid)
        palette = ["#5B8DEF", "#F5A623", "#7ED321", "#D0021B", "#9013FE", "#50E3C2",
                   "#B8860B", "#FF6F91", "#00A8CC", "#845EC2", "#4E8397", "#FF9671", "#008F7A"]

        fig11, axes11 = plt.subplots(n_rows_grid, n_cols_grid, figsize=(4.2 * n_cols_grid, 3.2 * n_rows_grid))
        axes_flat11 = axes11.flatten() if n > 1 else [axes11]

        bin_info_rows = []
        for i, (col_name, col_idx) in enumerate(numeric_cols_t11):
            vals = numeric_values(get_column(rows, col_idx))
            edges, width_or_freqs = compute_bins_manual(vals, n_bins_choice)
            freqs = compute_frequencies_manual(vals, edges)
            centers = [(edges[j] + edges[j + 1]) / 2 for j in range(len(edges) - 1)]
            bar_width = (edges[1] - edges[0]) * 0.92 if len(edges) > 1 else 1

            ax = axes_flat11[i]
            ax.bar(centers, freqs, width=bar_width, color=palette[i % len(palette)], edgecolor="white", linewidth=0.5, alpha=0.9)
            ax.set_title(col_name, fontsize=9, fontweight="bold")
            ax.tick_params(labelsize=7)
            ax.spines["top"].set_visible(False)
            ax.spines["right"].set_visible(False)

            bin_info_rows.append({"Column": col_name, "Bin Width": round(edges[1] - edges[0], 3), "Max Frequency": max(freqs)})

        for j in range(len(numeric_cols_t11), len(axes_flat11)):
            axes_flat11[j].axis("off")

        fig11.suptitle("Manually Binned Histograms — All Numeric Features", fontsize=13, fontweight="bold")
        fig11.tight_layout(rect=[0, 0, 1, 0.96])
        st.pyplot(fig11)
        plt.close(fig11)

        st.write("**Manual binning summary:**")
        st.table(bin_info_rows)

        with st.expander("📋 How binning & frequency counting was done manually"):
            st.markdown("""
- **Bin edges**: `width = (max − min) / n_bins`, then edges are generated as `min + i*width` for `i = 0..n_bins` — equivalent to what `numpy.histogram` does internally, but written out by hand (`compute_bins_manual`).
- **Frequency counting**: every value is scanned against the bin edges with a manual `lo <= v < hi` check (the last bin is inclusive on both ends so the maximum value isn't dropped) — see `compute_frequencies_manual`.
- **Drawing**: bars are placed at each bin's center using plain `ax.bar(centers, freqs, width=...)` — this is a raw drawing primitive, not a statistical plotting function like `ax.hist()`.
            """)

# ---------------- TASK 12 ----------------
with tab12:
    st.subheader("Manual Boxplots — Quartiles, IQR & Fences Computed by Hand")
    st.caption("No `ax.boxplot()` / `plt.boxplot()` — Q1/median/Q3, IQR, and Tukey fences are computed manually, and every box/whisker/outlier is drawn with raw `ax.fill_between` / `ax.plot` / `ax.scatter`.")

    numeric_cols_t12 = get_numeric_columns(header, rows)
    if not numeric_cols_t12:
        st.warning("No numeric columns found.")
    else:
        selected_cols_t12 = st.multiselect(
            "Columns to include in the boxplot",
            options=[c[0] for c in numeric_cols_t12],
            default=[c[0] for c in numeric_cols_t12][:6],
        )

        if selected_cols_t12:
            stats_by_col = {}
            for col_name in selected_cols_t12:
                col_idx = dict(numeric_cols_t12)[col_name]
                vals = numeric_values(get_column(rows, col_idx))
                stats_by_col[col_name] = compute_boxplot_stats_manual(vals)

            fig12 = draw_manual_boxplot(stats_by_col, selected_cols_t12)
            st.pyplot(fig12)
            plt.close(fig12)

            st.write("**Manually computed quartiles, IQR, fences & outlier counts:**")
            stats_table = []
            for col_name, s in stats_by_col.items():
                stats_table.append({
                    "Column": col_name,
                    "Q1": round(s["q1"], 2),
                    ...
                    "Q3": round(s["q3"], 2),
                    "IQR": round(s["iqr"], 2),
                    "Lower Fence": round(s["lower_fence"], 2),
                    "Upper Fence": round(s["upper_fence"], 2),
                    "Outliers": s["n_outliers"],
                })
            st.table(stats_table)

            total_outliers = sum(s["n_outliers"] for s in stats_by_col.values())
            st.info(f"**{total_outliers}** total outlier point(s) detected across the selected columns using Tukey's 1.5×IQR rule.")

            with st.expander("📋 How this was computed & drawn manually"):
                st.markdown("""
- **Q1 / Median / Q3**: computed with the same manual linear-interpolation percentile function used in Task 7 (`manual_percentile`), applied to a hand-sorted copy of the column (`manual_sort`).
- **IQR** = Q3 − Q1.
- **Fences** (Tukey's rule): `lower = Q1 − 1.5×IQR`, `upper = Q3 + 1.5×IQR`. Any value outside these bounds is flagged as an outlier.
- **Whiskers** extend only to the most extreme *non-outlier* value inside the fences (not simply to the fence itself), matching standard boxplot convention.
- **Drawing**, entirely with raw Matplotlib primitives:
  - the box is a filled rectangle from `ax.fill_between` between Q1 and Q3,
  - the median is a horizontal `ax.plot` line,
  - whiskers and their caps are short `ax.plot` line segments,
  - outliers are individual points via `ax.scatter`.
                """)
        else:
